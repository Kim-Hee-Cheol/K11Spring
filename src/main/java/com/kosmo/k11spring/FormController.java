...
사용자의 요청을 제일먼저 받는 DispatcherServlet은 기본
패키지로 지정된 com.kosmo.k11spring을 스캔하여 컨트롤러
클래스를 찾는다. 그리고 해당 요청명에 매핑되는 메소드를
찾아 요청을 전달하고 비지니스로직을 수행한다.
@Controller
	: 해당 클래스를 컨트롤러로 사용하고 싶을 때 클래스명앞에
	선언한다. 패키지를 스캔할 때 해당 어노테이션이 있는 클래스를
	찾아서 요청을 전달한다,
@RequestMapping
	: 요청명을 매핑한다. 요청명은 서블릿과 마찬가지로 컨택스트루트를
	제외한 나머지경로명으로 이루어진다. 요청명의 매핑정보로
	메소드를 호출하게되므로 메소드명은 큰 의미가 없다.
	개발자가 구분하기 좋은정도의 이름으로 설정하면 된다.
 */
@Controller
public class FormController {
	...
	폼값받기1] 파라미터로 전달된 값을 HttpServletRequest객체를
		통해서 받는다. Jsp나 Servlet에서 사용한방법과 동일하다.
	*/
	...
	/*
	폼값받기4] @PathVariable어노테이션으로 폼값받기
	요청명 ./form 뒤에 붙은 값이 메소드에서 사용가능한 파라미터가
	된다. 아래의 경우 2개의 파라미터를 받아서 사용하게된다.

...
public class DIController {
	...
	public String myCal(Model model) {
		...
		*/
		...
		/*
		XML설정파일에서 생성한 빈(Bean)을 geetBean()을 통해 주입받아
		참조변수에 할당한다. new연산자를 통해 생성한 것과 동일하지만
		외부 설정파일에서 미리 생성한것을 주입(Injection)받은 것임에
		유의할것.
		 */
	...
	public String bmiCal(Model model) {
		...
		//주입받은 빈을 토해 멤버메소드 호출 및 model객체에 정보저장
	...
	public ModelAndView myAvengers() {
		...
		아래 classpath 키워드는 Build Path메뉴에서 확인가능함.
		 */
		...
		//아이언맨 빈을 주입받은 후 정보출력을 위해 setter을 호출함
